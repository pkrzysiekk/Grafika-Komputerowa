...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener {
    ...
    private void drawCorkscrew(GL2 gl2) {
        ...
        var angle = Math.PI / n;// kształt spirali
    ...
    private void drawPiramid(GL2 gl2) {
        gl2.glScalef(2, 2, 2);
        gl2.glRotatef(45, 0, 1, 0);

        var points = new float[15][2];
        for (var i = 0; i < 15; i++) {
            var x = (float) (Math.cos(i * 2 * Math.PI / 15));
            var y = (float) (Math.sin(i * 2 * Math.PI / 15));

            points[i] = new float[] { x, y };
        }

        for (var i = 0; i < 15; i++)
            drawTriangle(points[(i == 0 ? points.length : i) - 1], points[i], gl2);
    }

    private void drawTriangle(float p1[], float p2[], GL2 gl2) {
        gl2.glColor3f(1, 0, 0);
        gl2.glBegin(GL_TRIANGLE_FAN);

        gl2.glVertex3d(p1[0], p1[1], 2f);
        gl2.glVertex3d(p2[0], p2[1], 2f);
