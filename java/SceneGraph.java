...
public class SceneGraph extends JPanel {
    ...
    private TransformedObject[] Shapes = new TransformedObject[7]; // increased the size to accommodate fifteenSidedPolygon
    ...
    private void createWorld() {
        ...
        Shapes[6] = new TransformedObject(fifteenSidedPolygon); // Added the fifteenSidedPolygon
        Shapes[6].setScale(0.5, 0.5).setTranslation(0, 0); // Adjust scale and translation as needed
        world.add(Shapes[6]);
    }
    ...
    private static SceneGraphNode hexagon = new SceneGraphNode() {
        void doDraw(Graphics2D g) {
            ...
            for (int i = 0; i < n; i++) {
                g.drawLine(xPoints[i], yPoints[i], 0, 0); //rysuje polaczenia w wielokacie
            }
    ...
    // New SceneGraphNode for a fifteen-sided polygon
    private static SceneGraphNode fifteenSidedPolygon = new SceneGraphNode() {
        void doDraw(Graphics2D g) {
            int n = 15; // liczba boków wielokąta
            double angle = (Math.PI * 2) / n; // kąt pomiędzy kolejnymi bokami

            int[] xPoints = new int[n];
            int[] yPoints = new int[n];

            for (int i = 0; i < n; i++) {
                xPoints[i] = (int) (Math.cos(i * angle) * 0.5 * 350); // współrzędna x na okręgu jednostkowym o promieniu 0.5 * 350
                yPoints[i] = (int) (Math.sin(i * angle) * 0.5 * 350); // współrzędna y na okręgu jednostkowym o promieniu 0.5 * 350
            }

            Polygon polygon = new Polygon(xPoints, yPoints, n); // wielokąt na podstawie wyliczonych punktów
            g.draw(polygon); // rysowanie wielokąta
        }
    };
