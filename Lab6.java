...
public class Main extends GLJPanel implements GLEventListener  {
	...
	public void display(GLAutoDrawable drawable) {
		...
		float[] zero = { 0, 0, 0, 1 };
		gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_AMBIENT_AND_DIFFUSE, materials[3], 0);
		...
		PyramidBaseOnCircle(gl2, 15);

	} // end display()

	public void PyramidBaseOnCircle(GL2 gl2, int num) {
		...
		for (int i = 1; i <= num; i++) {
			...
			gl2.glMaterialfv(GL2.GL_FRONT, GL2.GL_AMBIENT, materials[15],4);//gradient
...
//
	...
	private class MouseHandler extends MouseAdapter {
		...
		public void mousePressed(MouseEvent evt) {
			...
			prevX = evt.getX();
			dragging = true;
			Main.this.addMouseMotionListener(this);
		}
		public void mouseReleased(MouseEvent evt) {
			dragging = false;
			Main.this.removeMouseMotionListener(this);
		}
